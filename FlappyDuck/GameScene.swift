...
class GameScene: SKScene {
    ...
    override func update(_ currentTime: TimeInterval) {
        if gameStarted {
            var ySpeed = player.physicsBody!.velocity.dy * 0.001 as CGFloat
            player.zRotation = ySpeed
        ...
        if !gameFinished {
            ...
            if restart {
                ...
                scene.scaleMode = .aspectFill
                    view!.presentScene(scene, transition: .doorway(withDuration: 3))
            }
        }
    }
    ...
    func addFloor() {
        ...
        invisibleFloor.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: size.width, height: 1))
        invisibleFloor.physicsBody?.isDynamic = false
        invisibleFloor.position = CGPoint(x: size.width/2, y: size.height)
        invisibleFloor.zPosition = 2
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if gameStarted {
            if contact.bodyA.categoryBitMask == scoreCategory || contact.bodyB.categoryBitMask == scoreCategory {
                score += 1
                scoreLabel.text = "\(score)" 
            } else if contact.bodyA.categoryBitMask == pipeCategory || contact.bodyB.categoryBitMask ==
                pipeCategory {
                gameOver()
                run(gameOverSound)
            }
